...
export function BrandTab({ onOpenDefs }: { onOpenDefs: (anchor: string) => void }) {
    ...
    <div className="mx-auto max-w-[1120px] px-7 py-12">
      <div className="max-w-[64ch]">
        ...
        <h2 className="mt-2 text-[clamp(23px,3vw,30px)] font-semibold tracking-tight text-[var(--plg-ink)]">
          The whole-brand picture on {META.assistant}
        </h2>
      ...
      <div className="mt-2 grid grid-cols-1 gap-px overflow-hidden rounded-2xl border border-white/10 bg-white/10 md:grid-cols-3">
        <div className="bg-[var(--plg-bg)] p-6">
          <div className="flex items-center justify-between gap-2">
            <div className="text-[12.5px] font-semibold text-[var(--plg-muted)]">Brand visibility</div>
            <InfoDot onClick={() => onOpenDefs("def-01")} />
          </div>
        ...
        <div className="bg-[var(--plg-bg)] p-6">
          <div className="flex items-center justify-between gap-2">
            <div className="text-[12.5px] font-semibold text-[var(--plg-muted)]">
              Weighted share of voice
            </div>
            <InfoDot onClick={() => onOpenDefs("def-02")} />
          </div>
          <div className="mt-3 text-[44px] font-bold leading-none tracking-tight text-[var(--plg-ink)]">
            {brand.sov.toFixed(1)}
            <span className="ml-0.5 text-xl font-semibold text-[var(--plg-accent)]">%</span>
          ...
          <div className="mt-3 text-[13px] text-[var(--plg-text2)]">
            Position-weighted share of {META.assistant}&rsquo;s recommendations across all
            questions.
          </div>
          <div className="mt-4 h-1.5 overflow-hidden rounded-[3px] bg-white/[0.09]">
            <div
        ...
        <div className="bg-[var(--plg-bg)] p-6">
          <div className="flex items-center justify-between gap-2">
            ...
            <InfoDot onClick={() => onOpenDefs("def-03")} />
          </div>
          <div className="mt-3 text-[44px] font-bold leading-none tracking-tight text-[var(--plg-ink)]">
            {brand.rank?.toFixed(2) ?? "—"}
          </div>
          <div className="mt-3 text-[13px] text-[var(--plg-text2)]">
            Where {META.brand}&rsquo;s best-placed product typically lands. Lower is better.
          </div>
      ...
      <div className="mt-11">
        <div className="text-xs font-semibold uppercase tracking-[.14em] text-[var(--plg-accent)]">
          PLG-06b &middot; Alexa AI top 10
        </div>
        <h3 className="mt-2 text-[19px] font-semibold text-[var(--plg-ink)]">
          Where you stand against the field
        </h3>
        <p className="mt-2 max-w-[70ch] text-[14.5px] text-[var(--plg-text2)]">
          Top 10 brands on {META.assistant} across your selected topics, ranked by AI-shelf
          score — computed from the same {META.assistant} reads used above, no extra scrape.
        </p>
      </div>
